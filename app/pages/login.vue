      ...
      <Card class="max-w-xl">
        ...
        <CardContent class="flex flex-col items-center justify-center gap-4">
          <Form class="flex w-full flex-col gap-2" @submit="signInWithPassword(email, password)">
            <Input v-model="email" placeholder="Email" data-test-id="email" />
            <Input v-model="password" type="password" placeholder="Contraseña" data-test-id="password" />
            <Button v-if="!user" :disabled="email.length === 0 || isLoading || password.length === 0
          ...
          <div v-if="!user" class="flex flex-col items-center justify-center">
            <Button v-if="!user" variant="link" class="text-muted-foreground w-full text-xs hover:cursor-pointer"
              :disabled="isLoading" @click="signInAnonymous">
              Ingresa como invitado
            </Button>

            <Button v-if="!user" as-child variant="link"
          ...
          <p v-if="errorMessage" class="text-destructive-foreground dark:text-destructive-foreground text-xs">
            {{ errorMessage }}
          </p>
        </CardContent>
      </Card>
    ...
    <TabsContent value="sign">
      <Card class="max-w-xl">
        ...
        <CardContent class="flex flex-col items-center justify-center gap-4">
          ...
          <Button variant="link" class="text-muted-foreground w-full cursor-pointer" :disabled="auth.isLoading.value"
